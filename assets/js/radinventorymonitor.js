...
var radiology = {
    get_inventory_monitoring : function(){
        start_date = $('#search-rad-inventory-form input[name=start_date]').val(); 
        end_date   = $('#search-rad-inventory-form input[name=end_date]').val(); 
         
        $("#search-rad-inventory-form input[name=s_date]").val(start_date);
        $("#search-rad-inventory-form input[name=e_date]").val(end_date);
//        alert($("#search-hmo-asofdate-form input[name=s_date]").val());
//        alert(hmolist);
        $('#rad-inventory-table').dataTable().fnDestroy();
        var table = $('#rad-inventory-table').dataTable({
            dom: 'frtip',
          //  responsive: true,
          pageLength:10,
          pagination: true,
            ...
            ajax:{  
                url:BASE_URL + "user/fetch_rad_inventory_monitoring",
                type:"POST",
                 data: {start_date: $("#search-rad-inventory-form input[name=s_date]").val() 
...
//                }
        ...
        $('#rad-inventory-table_filter input').unbind();
        $('#rad-inventory-table_filter input').bind('keyup', function(e) {
            if(e.keyCode == 13) {
    ...
    search_inventory_monitoring : function(){
        ...
        var s_date = $('#search-rad-inventory-form input[name=start_date]');
        var e_date = $('#search-rad-inventory-form input[name=end_date]');
        var error = 0;
...
//            hmo.get_hmo_asof_date(hmocodex[0]);
        radiology.get_inventory_monitoring();
    ...
    generate_inventory_monitoring_report : function(){
        ...
        $("#rad-inventory-form input[name=start_date]").val(start_date);
        $("#rad-inventory-form input[name=beginning]").val(beginning);
        $("#rad-inventory-form input[name=sales]").val(totalsales);
        $("#rad-inventory-form input[name=ending]").val(totalending);
        $("#rad-inventory-form").submit();
...
$('#rad-inventory-table').on('click', 'td', function () {
    ...
    var data = $('#rad-inventory-table').dataTable().fnGetData(current_row);
    var start_date = $('#search-rad-inventory-form input[name=start_date]').val(); 
    ...
    var currentYear = moment(new Date(start_date)).format('YYYY');
     ...
     $('#inventorymonitor-group-table').dataTable().fnDestroy();
        var table = $('#inventorymonitor-group-table').dataTable({
            ...
            order:[],  
            ajax:{  
                url:BASE_URL + "user/fetch_groupname_info_rad",
                type:"POST",
                data: { 
                    groupname: data[0],
                    s_date: start_date
