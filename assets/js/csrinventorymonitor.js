...
var csr = {
    get_inventory_monitoring : function(){
        start_date = $('#search-csr-inventory-form input[name=start_date]').val(); 
        end_date   = $('#search-csr-inventory-form input[name=end_date]').val(); 
         
        $("#search-csr-inventory-form input[name=s_date]").val(start_date);
        $("#search-csr-inventory-form input[name=e_date]").val(end_date);
//        alert($("#search-hmo-asofdate-form input[name=s_date]").val());
//        alert(hmolist);
        $('#csr-inventory-table').dataTable().fnDestroy();
        var table = $('#csr-inventory-table').dataTable({
            dom: 'frtip',
          //  responsive: true,
          pageLength:10,
          pagination: true,
            ...
            ajax:{  
                url:BASE_URL + "user/fetch_csr_inventory_monitoring",
                type:"POST",
                 data: {start_date: $("#search-csr-inventory-form input[name=s_date]").val() 
...
//                }
        ...
        $('#csr-inventory-table_filter input').unbind();
        $('#csr-inventory-table_filter input').bind('keyup', function(e) {
            if(e.keyCode == 13) {
    ...
    search_inventory_monitoring : function(){
        ...
        var s_date = $('#search-csr-inventory-form input[name=start_date]');
        var e_date = $('#search-csr-inventory-form input[name=end_date]');
        var error = 0;
...
//            hmo.get_hmo_asof_date(hmocodex[0]);
    ...
    generate_inventory_monitoring_report : function(){
        ...
        $("#csr-inventory-form input[name=start_date]").val(start_date);
        $("#csr-inventory-form input[name=beginning]").val(beginning);
        $("#csr-inventory-form input[name=sales]").val(totalsales);
        $("#csr-inventory-form input[name=ending]").val(totalending);
        $("#csr-inventory-form").submit();
...
$('#csr-inventory-table').on('click', 'td', function () {
    ...
    var data = $('#csr-inventory-table').dataTable().fnGetData(current_row);
    var start_date = $('#search-csr-inventory-form input[name=start_date]').val(); 
    ...
    var currentYear = moment(new Date(start_date)).format('YYYY');
     ...
     $('#inventorymonitor-group-table').dataTable().fnDestroy();
        var table = $('#inventorymonitor-group-table').dataTable({
            ...
            order:[],  
            ajax:{  
                url:BASE_URL + "user/fetch_groupname_info_csr",
                type:"POST",
                data: { 
                    groupname: data[0],
                    s_date: start_date
