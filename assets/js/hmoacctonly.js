...
var hmoacct = {
    get_hmo_acct : function(hmolist){
        start_date = $('#search-hmo-acct-form input[name=start_date]').val(); 
         end_date = $('#search-hmo-acct-form input[name=end_date]').val(); 
         
        $("#search-hmo-acct-form input[name=s_date]").val(start_date);
        $("#search-hmo-acct-form input[name=e_date]").val(end_date);
//        alert($("#search-hmo-asofdate-form input[name=s_date]").val());
//        alert(hmolist);
        $('#hmo-acct-table').dataTable().fnDestroy();
        var table = $('#hmo-acct-table').dataTable({
            dom: 'frtip',
          //  responsive: true,
          pageLength:10,
          pagination: true,
            ...
            ajax:{  
                url:BASE_URL + "user/fetch_hmo_acct_patients",
                type:"POST",
                 data: {start_date: $("#search-hmo-acct-form input[name=s_date]").val(), 
                        end_date: $("#search-hmo-acct-form input[name=e_date]").val(), 
                        hmocode: hmolist}
        ...
        $('#hmo-acct-table_filter input').unbind();
        $('#hmo-acct-table_filter input').bind('keyup', function(e) {
            if(e.keyCode == 13) {
    ...
    search_hmo_acct : function(){
        var s_date = $('#search-hmo-acct-form input[name = start_date]');
        var e_date = $('#search-hmo-acct-form input[name = end_date]');
        var error = 0;
        ...
        if (error === 0) {
            ...
            hmoacct.get_hmo_acct(hmocodex[0]);
           
        }
    },
    
    generate_hmoacct_report : function(){
        ...
        $("#hmo-acct-form input[name=start_date]").val(start_date);
        $("#hmo-acct-form input[name=end_date]").val(end_date);
        $("#hmo-acct-form input[name=hmocodex]").val(hmocodex[0]);
        $("#hmo-acct-form input[name=hmonamex]").val(hmocodex[1]);
        $("#hmo-acct-form input[name=hospital]").val(hospitalfee);
        $("#hmo-acct-form input[name=proffee]").val(proffee);
         
        $("#hmo-acct-form").submit();
...
function load_hmoacctonly_script(scriptUrl)
{
    $.getScript(scriptUrl)
